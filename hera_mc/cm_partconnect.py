...
class FengineStatus(MCDeclarativeBase):
    """
    A table of
    ...
    hostname : String Column
        F-engine hostname (e.g. heraNode2Snap1)
    node : int
        Node number (redundant from hostname)
    position : int
        SNAP position in node [0, 1, 2, 3] (redundant from hostname)
    ants : String Column
        csv-list of f-engine antennas
    ant_indices : String Column
        csv-list of correlator antenna indices
    phase_indices : String Column
        csv-list of phase switch indices
    comment : String Column
        f-engine change comment
    start_gpstime : int
    ...
    __tablename__ = 'fengine_status'

    hostname = Column(String(64), primary_key=True)
    node = Column(BigInteger, nullable=False)
    position = Column(BigInteger, nullable=False)
    ants = Column(String(64))
    ant_indices = Column(String(64))
    phase_indices = Column(String(64))
    ...
    def __repr__(self):
        ...
        return ('<{self.hostname}  -  {self.comment} :: {self.start_gpstime} - '
                '{self.stop_gpstime}>'.format(self=self))


def update_fengine_status(hostname, node, position, at_date, ants=None, ant_indices=None,
                          phase_indices=None, comment=None, date2=None, session=None):
    """
    Update
    ...
    hostname : String Column
        F-engine hostname (e.g. heraNode2Snap1)
    node : int
        Node number (redundant from hostname - use-rosetta)
    snap_loc_num : int
        SNAP position in node [0, 1, 2, 3] (redundant from hostname)
    ants : String Column
        csv-list of f-engine antennas
