    ...
    def show_connections(self, connection_dossier, verbosity='h'):
        ...
        for ordered_pairs in connection_dossier['ordered-pairs']:
            for up, dn in zip(ordered_pairs[0], ordered_pairs[1]):
                ...
                show_conn_dict['Part'] = connup.downstream_part + ':' + connup.down_part_rev
                show_conn_dict['Upstream'] = connup.upstream_part + ':' + connup.up_part_rev
                if PC.no_connection_designator in show_conn_dict['Upstream']:
                    show_conn_dict['uStart'] = PC.no_connection_designator
                    show_conn_dict['uStop'] = PC.no_connection_designator
                else:
                ...
                if conndn.upstream_part + ':' + conndn.up_part_rev != show_conn_dict['Part']:
                    print("Warning:  Part {} does not match.".format(conn_dict['Part']))
                show_conn_dict['Downstream'] = conndn.downstream_part + ':' + conndn.down_part_rev
                if PC.no_connection_designator in show_conn_dict['Downstream']:
                    show_conn_dict['dStart'] = PC.no_connection_designator
                    show_conn_dict['dStop'] = PC.no_connection_designator
                else:
