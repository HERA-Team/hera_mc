...
def task_multiple(observation):
    class DataHolder(object):
        ...
        t0 = Time(2457000, format="jd")
        time = t0 - TimeDelta(30 * 60, format='sec')
        task_resource_names = ['obsid_start', 'task_name', 'start_time', 'stop_time',
                               'max_memory', 'avg_cpu_load']
        task_resource_values = [observation.obsid, 'OMNICAL', time,
                                time + TimeDelta(10 * 60, format='sec'),
                                16.2, 1.01]
...
def test_rtp_task_jobid(mcsession, observation, multiple, jobid, multiple_jobid):
    ...
    get_keywords = {
        "starttime": data_obj.task_jobid_columns['start_time'] - TimeDelta(2, format='sec'),
    ...
    new_jobid_time = (data_obj.task_jobid_columns['start_time']
                      + TimeDelta(60, format='sec'))
    new_task = 'task2'
    add_method(
        data_obj.task_jobid_columns[obsid_name], new_task, new_jobid_time,
    ...
    get_keywords["stoptime"] = (
        data_obj.task_jobid_columns['start_time'] + TimeDelta(2 * 60, format='sec')
    )
    result = get_method(**get_keywords)
    assert len(result) == 2
    ...
    new_task_time = (data_obj.task_jobid_columns['start_time']
                     + TimeDelta(3 * 60, format='sec'))

    new_obsid_time = (data_obj.task_jobid_columns['start_time']
                      + TimeDelta(3 * 60, format='sec'))
    new_obsid = utils.calculate_obsid(new_obsid_time)
    test_session.add_obs(
        Time(new_obsid_time),
    ...
    add_method(
        new_obsid, data_obj.task_jobid_columns['task_name'], new_task_time,
        *data_obj.task_jobid_values[3:])
    ...
    get_keywords = {
        ...
        obsid_name: data_obj.task_jobid_columns[obsid_name],
        "task_name": data_obj.task_jobid_columns['task_name'],
        "stoptime": data_obj.task_jobid_columns['start_time'] + TimeDelta(5 * 60, format='sec')
    }
    result = get_method(**get_keywords)
    assert len(result) == 1
    result = result[0]
...
def test_errors_rtp_task_jobid(mcsession, observation, multiple, jobid, multiple_jobid):
    ...
    with pytest.raises(
        ValueError,
        match=(
            'If most_recent is set to False, at least one of ' + obsid_name
...
def test_add_rtp_task_resource_record(mcsession, observation, multiple, task, task_multiple):
    ...
    get_keywords = {
        "starttime": data_obj.task_resource_columns['start_time'] - TimeDelta(2, format='sec'),
    ...
    new_task_time = (data_obj.task_resource_columns['start_time']
                     + TimeDelta(60, format='sec'))
    new_task = 'task2'
    add_method(
        data_obj.task_resource_columns[obsid_name], new_task, new_task_time,
    ...
    get_keywords["stoptime"] = (
        data_obj.task_resource_columns['start_time'] + TimeDelta(2 * 60, format='sec')
    )
    result = get_method(**get_keywords)
    assert len(result) == 2
    ...
    new_obsid_time = (data_obj.task_resource_columns['start_time']
                      + TimeDelta(3 * 60, format='sec'))
    new_obsid = utils.calculate_obsid(new_obsid_time)
    test_session.add_obs(
        Time(new_obsid_time),
    ...
    add_method(
        new_obsid, data_obj.task_resource_columns['task_name'], new_task_time,
        *data_obj.task_resource_values[3:])
    ...
    get_keywords = {
        ...
        obsid_name: data_obj.task_resource_columns[obsid_name],
        "task_name": data_obj.task_resource_columns['task_name'],
        "stoptime": data_obj.task_resource_columns['start_time'] + TimeDelta(5 * 60, format='sec')
    }
    result = get_method(**get_keywords)
    assert len(result) == 1
    result = result[0]
...
@pytest.mark.parametrize("multiple", [False, True])
def test_errors_rtp_task_resource_record(mcsession, observation, multiple, task, task_multiple):
    test_session = mcsession
    test_session.add_obs(*observation.observation_values)
    obs_result = test_session.get_obs()
    assert len(obs_result) == 1
    ...
    with pytest.raises(
        ValueError,
        match=(
            'If most_recent is set to False, at least one of ' + obsid_name + ', task_name '
...
def test_add_rtp_task_multiple_track(mcsession, observation, task_multiple, multiple_track):
    ...
    test_session.add_rtp_task_multiple_resource_record(*task_multiple.task_resource_values)
