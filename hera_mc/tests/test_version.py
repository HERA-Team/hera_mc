...
def test_construct_version_info():
    # this test is a bit silly because it uses the nearly the same code as the
    # original, but it will detect accidental changes that could cause problems.
    # It does test that the __version__ attribute is set on hera_mc.

    # this line is modified from the main implementation since we're in
    # hera_mc/tests/
    hera_mc_dir = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    ...
    test_version_info = {'version': hera_mc.__version__,
    ...
    assert hera_mc.version.construct_version_info() == test_version_info
