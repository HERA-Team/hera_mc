...
def test_reduce_time_vals():
    ...
    assert np.allclose(dec_vals, np.array(range(60, 156, 60)) + .24)
    ...
    assert np.allclose(dec_vals, np.array(range(33, 156, 3)) + .24)

    pytest.raises(ValueError, weather._reduce_time_vals, times, values, 1.5)

    # test mean strategy
    dec_times, dec_vals = weather._reduce_time_vals(times, values, 10,
                                                    strategy='mean')
    ...
    # test sum strategy
    dec_times, dec_vals = weather._reduce_time_vals(
        times, values, 10, strategy='sum')
    ...
    # test error
    pytest.raises(ValueError, weather._reduce_time_vals, times, values, 10,
                  strategy='foo')
...
def test_add_weather(mcsession):
    ...
    expected = [weather.WeatherData(time=int(floor(t1.gps)),
                                    variable='wind_speed',
                                    value=wind_speeds[0]),
    ...
    result_var_most_recent = test_session.get_weather_data(
        variable='temperature')
...
def test_add_from_sensor(mcsession):
    ...
    pytest.raises(ValueError, weather.create_from_sensors, t1, t2,
                  variables='foo')


def test_weather_errors(mcsession):
    test_session = mcsession
    pytest.raises(ValueError, test_session.add_weather_data, 'foo',
                  'wind_speed', 2.548)

    t1 = Time('2017-11-10 01:15:23', scale='utc')
    pytest.raises(ValueError, test_session.add_weather_data, t1,
                  'foo', 2.548)
    ...
    pytest.raises(ValueError, test_session.add_weather_data_from_sensors,
...
def test_dump_weather_table(mcsession):
    ...
    for var in ['wind_speed', 'wind_direction', 'temperature']:
        filename = var + '.txt'
        test_session.get_weather_data(starttime=t1, stoptime=t2, variable=var,
