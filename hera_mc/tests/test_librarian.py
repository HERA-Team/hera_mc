...
def test_add_lib_status(mcsession):
    test_session = mcsession
    test_session.add_lib_status(*status_values)
    ...
    test_session.add_lib_status(
        new_status_time, status_columns['num_files'] + 2,
        status_columns['data_volume_gb'] + .2,
        status_columns['free_space_gb'] - .2,
        2.2, status_columns['num_processes'],
        status_columns['git_version'],
        status_columns['git_hash'])
...
def test_errors_lib_status(mcsession):
    test_session = mcsession
...
def test_add_raid_status(mcsession):
    test_session = mcsession
    exp_columns = raid_status_columns.copy()
    exp_columns['time'] = int(floor(exp_columns['time'].gps))
    expected = LibRAIDStatus(**exp_columns)

    test_session.add_lib_raid_status(*raid_status_values)

    result = test_session.get_lib_raid_status(
        starttime=raid_status_columns['time'] - TimeDelta(2 * 60, format='sec'))
    assert len(result) == 1
    result = result[0]
    ...
    test_session.add_lib_raid_status(
        raid_status_values[0], 'raid_2', *raid_status_values[2:])
    result_host = test_session.get_lib_raid_status(
        starttime=raid_status_columns['time'] - TimeDelta(2, format='sec'),
        hostname=raid_status_columns['hostname'],
        stoptime=raid_status_columns['time'] + TimeDelta(2 * 60, format='sec'))
...
def test_errors_lib_raid_status(mcsession):
    test_session = mcsession
    pytest.raises(ValueError, test_session.add_lib_raid_status,
                  'foo', *raid_status_values[1:])
    ...
    pytest.raises(ValueError, test_session.get_lib_raid_status,
                  starttime=raid_status_columns['time'], stoptime='foo')


def test_add_raid_error(mcsession):
    test_session = mcsession
    exp_columns = raid_error_columns.copy()
    exp_columns['time'] = int(floor(exp_columns['time'].gps))
    expected = LibRAIDErrors(**exp_columns)

    test_session.add_lib_raid_error(*raid_error_values[1:])

    result = test_session.get_lib_raid_error(
        starttime=raid_error_columns['time'] - TimeDelta(2 * 60, format='sec'))
    assert len(result) == 1
    result = result[0]

    assert result.isclose(expected)

    test_session.add_lib_raid_error(raid_error_values[1], 'raid_2',
                                    *raid_error_values[3:])
    result_host = test_session.get_lib_raid_error(
        starttime=raid_error_columns['time'] - TimeDelta(2, format='sec'),
        hostname=raid_error_columns['hostname'],
        stoptime=raid_error_columns['time'] + TimeDelta(2 * 60, format='sec'))
    assert len(result_host) == 1
    result_host = result_host[0]
    assert result_host.isclose(expected)
...
def test_errors_lib_raid_error(mcsession):
    test_session = mcsession
    pytest.raises(ValueError, test_session.add_lib_raid_error,
                  'foo', *raid_error_values[2:])
    ...
    pytest.raises(ValueError, test_session.get_lib_raid_error,
                  starttime=raid_error_columns['time'], stoptime='foo')


def test_add_remote_status(mcsession):
    test_session = mcsession
    exp_columns = remote_status_columns.copy()
    exp_columns['time'] = int(floor(exp_columns['time'].gps))
    expected = LibRemoteStatus(**exp_columns)

    test_session.add_lib_remote_status(*remote_status_values)

    result = test_session.get_lib_remote_status(
        starttime=remote_status_columns['time']
    ...
    test_session.add_lib_remote_status(remote_status_values[0], 'penn',
                                       *remote_status_values[2:])
    result_remote = test_session.get_lib_remote_status(
        starttime=remote_status_columns['time'] - TimeDelta(2, format='sec'),
        remote_name=remote_status_columns['remote_name'],
        stoptime=remote_status_columns['time']
        + TimeDelta(2 * 60, format='sec'))
...
def test_errors_lib_remote_status(mcsession):
    test_session = mcsession
    pytest.raises(ValueError, test_session.add_lib_remote_status,
                  'foo', *remote_status_values[1:])

    test_session.add_lib_remote_status(*remote_status_values)
    pytest.raises(ValueError, test_session.get_lib_remote_status,
                  starttime='foo')
    pytest.raises(ValueError, test_session.get_lib_remote_status,
                  starttime=remote_status_columns['time'], stoptime='foo')


def test_add_lib_file(mcsession):
    test_session = mcsession
    # raise error if try to add process event with unmatched obsid
    # pytest.raises(NoForeignKeysError, test_session.add_lib_file,
    #                   file_values[0], file_values[1] + 2,
    #                   file_values[2:5])
    ...
    test_session.add_lib_file(*file_values)

    exp_columns = file_columns.copy()
    exp_columns['time'] = int(floor(exp_columns['time'].gps))
    expected = LibFiles(**exp_columns)

    result_file = test_session.get_lib_files(
        filename=file_columns['filename'])[0]
    assert result_file.isclose(expected)

    result = test_session.get_lib_files(
        starttime=file_columns['time'] - TimeDelta(2, format='sec'))
    assert len(result) == 1
    result = result[0]
    assert result.isclose(expected)
    ...
    result_obsid = test_session.get_lib_files(
        starttime=file_columns['time'] - TimeDelta(2, format='sec'),
        obsid=file_columns['obsid'])
    assert len(result_obsid) == 1
    result_obsid = result_obsid[0]
    assert result_obsid.isclose(expected)

    new_file_time = file_columns['time'] + TimeDelta(3 * 60, format='sec')
    new_file = 'file2'
    test_session.add_lib_file(new_file, file_values[1], new_file_time, 1.4)
    result_obsid = test_session.get_lib_files(obsid=file_columns['obsid'])
    assert len(result_obsid) == 2

    filename = os.path.join(DATA_PATH, 'test_lib_file_record_file.csv')
    test_session.get_lib_files(obsid=file_columns['obsid'],
...
def test_lib_file_null_obsid(mcsession):
    test_session = mcsession
    test_session.add_obs(*observation_values)
    ...
    result = test_session.get_lib_files(filename=file_values[0])
    assert len(result) == 1
...
def test_errors_add_lib_file(mcsession):
    test_session = mcsession
    test_session.add_obs(*observation_values)
    obs_result = test_session.get_obs()
    assert len(obs_result) == 1

    pytest.raises(ValueError, test_session.add_lib_file, status_values[0],
                  status_values[1], 'foo', status_values[3])

    test_session.add_lib_file(*file_values)
    pytest.raises(ValueError, test_session.get_lib_files, starttime='foo')
    pytest.raises(ValueError, test_session.get_lib_files,
                  starttime=file_columns['time'], stoptime='bar')
