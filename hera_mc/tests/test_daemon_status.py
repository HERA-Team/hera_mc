...
def test_add_daemon_status(mcsession, daemon_data):
    ...
    test_session.commit()
    test_session.add_daemon_status(column_values[0], column_values[1],
                                   Time.now(), 'errored')
    test_session.commit()
    result = test_session.get_daemon_status(
        starttime=columns['time'] - TimeDelta(2, format='sec'))
...
def test_errors_daemon_status(mcsession, daemon_data):
    test_session = mcsession
    column_values, columns = daemon_data
    pytest.raises(ValueError, test_session.add_daemon_status,
                  column_values[0], column_values[1], 'foo', column_values[3])

    pytest.raises(ValueError, test_session.add_daemon_status,
                  column_values[0], column_values[1], column_values[2], 'foo')

    test_session.add_daemon_status(*column_values)
    pytest.raises(ValueError, test_session.get_daemon_status,
                  starttime='test_host')
    pytest.raises(ValueError, test_session.get_daemon_status,
                  starttime=columns['time'], stoptime='test_host')
