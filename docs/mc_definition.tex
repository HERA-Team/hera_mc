...
When correlator components had an event (related to normal observing).
\begin{center}
 \begin{tabular}{| p{4cm} | p{2cm} | p{10cm} |}
\hline
 {\bf Column} & {\bf Type}  & {\bf Description} \\ [0.5ex]  \hline\hline
\textbf{component\_event} & string & Correlator event, one of :``f\_engine\_sync", ``x\_engine\_integration" (in this case it is calculated from the Mcount) ``catcher\_start", ``catcher\_stop".
\\ \hline
