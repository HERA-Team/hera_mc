...
F-engine initialization status of the SNAP, status can be ``working", ``unconfig" (configuration did not work, so SNAP does not work), or ``maxout" (snap configured properly but was excluded because there were too many SNAPs).
\begin{center}
 \begin{tabular}{| p{4cm} | p{2cm} | p{10cm} |}
\hline
 {\bf Column} & {\bf Type}  & {\bf Description} \\ [0.5ex]  \hline\hline
\textbf{time} & long & status time in floor(gps seconds). \\ \hline
\textbf{hostname} & string & SNAP hostname. \\ \hline
status & string & One of ``working"  ``unconfig" or ``maxout". \\ \hline
\end{tabular}
\end{center}
