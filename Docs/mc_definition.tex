...
{\tt geo.py fg_action --arguments}

Has various plotting/printing options for station information.  {{\tt fg_action} is the "foreground action", that is the locations that will get printed and shown on top if plotting.
Available foregrounds are (note you need the first letter only):  a[ctive], i[nstalled], p[osition] <csv-list>, c[ofa], s[ince], n[one].
