...
def get_part_information(args, install):
    ...
    try:
        ...
        print('\nError:  ', args.hptype,
              'not valid part type -- look again at the list\n')
        install.get_part_types(args, True)
        print()
        sys.exit()

    new_or_replace = raw_input(
        "Is this a 'n'ew or 'r'eplacement part (n/r)?  ").lower()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--revision_number',
                        help="Revision number of part", default='last')
    parser.add_argument('-q', '--question',
                        help="Flag to force prompting.", action='store_true')
    parser.add_argument('-v', '--verbosity',
                        help="Set verbosity {l, m, h}. [h].", default="h")
    parser.add_argument(
        '-c', '--connection', help="Show all connections directly to a part. [None]", default=None)
    parser.add_argument(
        '-u', '--update', help="Update part number records.  Format hpn0:col0:val0, [hpn1:]col1:val1...  [None]", default=None)
    parser.add_argument(
